...
module.exports = exports = class Device extends EventEmitter {
  ...
  // `security` only bites once the peer is bonded: the kernel then encrypts
  // with the stored LTK (smp.c smp_ltk_encrypt). Without a bond it logs
  // "security requested but not available" and connects in the clear
  // (smp.c smp_conn_security). The listening side is always enforced.
