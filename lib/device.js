...
module.exports = exports = class Device extends EventEmitter {
  ...
  // it then encrypts with the stored LTK (smp.c smp_ltk_encrypt). Without a
  // bond it would log "security requested but not available" and connect in
  // the clear (smp.c smp_conn_security), so refuse instead of pretending.
  // The listening side is always enforced by the kernel.
  openL2CAPChannel(psm, { security } = {}) {
    if (this._adapter._destroyed) return
    if (['medium', 'high', 'fips'].includes(security) && !this.paired) {
      queueMicrotask(() => {
