...
module.exports = exports = class Adapter extends EventEmitter {
  ...
  setDiscoveryFilter({ uuids = [], rssi, transport } = {}) {
    ...
    return new Promise((resolve, reject) => {
      binding.adapterSetDiscoveryFilter(this._handle, uuids, rssi, transport, (err) => {
        if (err) reject(err)
        else resolve()
      })
    })
  ...
  startDiscovery() {
    ...
    return new Promise((resolve, reject) => {
      binding.adapterStartDiscovery(this._handle, (err) => {
        if (err) reject(err)
        else resolve()
      })
    })
  ...
  stopDiscovery() {
    ...
    return new Promise((resolve, reject) => {
      binding.adapterStopDiscovery(this._handle, (err) => {
        if (err) reject(err)
        else resolve()
      })
    })
  ...
  removeDevice(device) {
    ...
    return new Promise((resolve, reject) => {
      binding.deviceRemove(this._handle, device.path, (err) => {
        if (err) reject(err)
        else resolve()
      })
    })
  ...
  registerAgent(agent, capability = '') {
    ...
    return new Promise((resolve, reject) => {
      binding.agentRegister(this._handle, capability, (err) => {
        if (err) {
          reject(err)
        } else {
          this._agent = agent
  ...
  // BlueZ is waiting on a reply here, so throwing would leave it hanging until
  // its own timeout. Failures go back as D-Bus errors, not up to a caller
  async _onagentrequest(method, id, device, pincode, uuid, passkey, entered) {
    if (this._destroyed) return
    ...
    try {
      switch (method) {
        ...
        case 'DisplayPinCode':
          await agent.displayPinCode(device, pincode)
          break
        case 'DisplayPasskey':
          return await agent.displayPasskey(device, passkey, entered)
        case 'RequestPinCode': {
          const pinCode = await agent.requestPinCode(device)
          if (this._destroyed) return
          return binding.agentRespondString(this._handle, id, String(pinCode))
        }
        case 'RequestPasskey': {
          const passkey = await agent.requestPasskey(device)
          if (this._destroyed) return
          return binding.agentRespondNumber(this._handle, id, Number(passkey))
        }
        case 'RequestConfirmation':
          refused = (await agent.requestConfirmation(device, passkey)) === false
          break
        case 'RequestAuthorization':
          refused = (await agent.requestAuthorization(device)) === false
          break
        case 'AuthorizeService':
          refused = (await agent.authorizeService(device, uuid)) === false
          break
        default:
          return
      }

      if (refused) throw new Error(method + ' refused by the agent')
  ...
  _ongattcharacteristicwrite(path, id, value, offset, type, mtu, device, link, prepareAuthorize) {
    if (!this._gattApp) return
    for (const svc of this._gattApp.services) {
      for (const ch of svc.characteristics) {
        if (ch._path === path) {
          ch._onwrite(id, new Uint8Array(value), {
            offset,
            type,
            mtu,
            device,
