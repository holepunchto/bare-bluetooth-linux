...
module.exports = exports = class Adapter extends EventEmitter {
  ...
  _ongattcharacteristicread(path, id, offset, device) {
    if (!this._gattApp) return
    for (const svc of this._gattApp.services) {
      for (const ch of svc.characteristics) {
        if (ch._path === path) {
          ch._onread(id, { offset, device })
          return
        }
      }
    }
  ...
  _ongattcharacteristicwrite(path, value, offset, device) {
    if (!this._gattApp) return
    for (const svc of this._gattApp.services) {
      for (const ch of svc.characteristics) {
        if (ch._path === path) {
          ch._value = value
          ch.emit('write', value, { offset, device })
          return
        }
      }
    }
