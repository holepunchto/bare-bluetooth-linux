...
const GATT_APP_PATH = '/com/bare/gatt'
...
module.exports = exports = class Adapter extends EventEmitter {
  ...
  registerApplication(application) {
    ...
    for (let i = 0; i < application.services.length; i++) {
      ...
      const svcIdx = binding.gattServiceAdd(this._handle, svc.uuid, svc.primary)

      for (let j = 0; j < svc.characteristics.length; j++) {
        const ch = svc.characteristics[j]
        const chIdx = binding.gattCharacteristicAdd(
          this._handle,
          svcIdx,
          ch.uuid,
          ch.flags,
        ...
        ch._adapter = this
        ch._serviceIndex = svcIdx
        ch._characteristicIndex = chIdx
      }
    }
  ...
  _ongattcharacteristicwrite(path, value) {
    ...
    for (const svc of this._gattApp.services) {
      for (const ch of svc.characteristics) {
        if (
          ch._adapter &&
          path === GATT_APP_PATH + '/service' + ch._serviceIndex + '/char' + ch._characteristicIndex
        ) {
          ch._value = value
          ch.emit('write', value)
          return
