    ...
    try {
      switch (method) {
        ...
        case 'Cancel':
          return agent.cancel()
        case 'DisplayPinCode':
          return agent.displayPinCode(device, text)
        case 'DisplayPasskey':
          return agent.displayPasskey(device, number, entered)
        case 'RequestPinCode':
          return binding.agentRespondString(this._handle, id, await agent.requestPinCode(device))
        case 'RequestPasskey':
          return binding.agentRespondNumber(this._handle, id, await agent.requestPasskey(device))
      ...
      if (id !== 0) {
        binding.agentRespondError(this._handle, id, 'org.bluez.Error.Rejected', err.message)
      }
    }
  }
