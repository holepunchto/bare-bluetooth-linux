...
module.exports = exports = class Adapter extends EventEmitter {
  ...
  publishL2CAPChannel(opts = {}) {
    if (this._destroyed) return
    const { psm = 0 } = opts

    // The native callback fires synchronously; defer the events so callers
    // can attach listeners after calling publish, like on the other platforms
    binding.l2capPublish(this._handle, psm, this, this._onl2capconnection, (err, serverHandle) => {
      queueMicrotask(() => {
        if (err || !serverHandle) {
        ...
        const assigned = binding.l2capServerPsm(serverHandle)
        this._l2capServers.set(assigned, serverHandle)
        this.emit('channelPublish', assigned)
      })
    })
  }

  unpublishL2CAPChannel(psm) {
    const serverHandle = this._l2capServers.get(psm)
    if (!serverHandle) return
    this._l2capServers.delete(psm)
    binding.l2capUnpublish(serverHandle)
  }
